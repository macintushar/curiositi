...
4. Queue invokes the Worker:
   - **QStash**: Sends a signed HTTP POST to `/process-file` with the job payload
   - **bunqueue**: Worker polls the bunqueue server for jobs and processes them
...
- `QUEUE_PROVIDER=qstash`
- `QSTASH_TOKEN` is set (used by the platform to enqueue jobs)
- `QSTASH_CURRENT_SIGNING_KEY` and `QSTASH_NEXT_SIGNING_KEY` are set (used by the worker to verify incoming requests)
...
| `QSTASH_TOKEN` | qstash | Conditional | - | Upstash QStash authentication token (platform) |
| `QSTASH_URL` | qstash | No | Upstash default | Custom QStash endpoint URL |
| `QSTASH_CURRENT_SIGNING_KEY` | qstash | Conditional | - | Current signing key for request verification (worker) |
| `QSTASH_NEXT_SIGNING_KEY` | qstash | Conditional | - | Next signing key for request verification (worker) |
| `WORKER_URL` | qstash | Conditional | - | Public URL of the worker service (required by platform) |
