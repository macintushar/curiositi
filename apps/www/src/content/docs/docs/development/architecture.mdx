...
│                      Application Layer                          │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │              Platform (TanStack Start)                   │   │
│  │  ┌────────────┐ ┌────────────┐ ┌────────────────────┐    │   │
│  │  │   Routes   │ │ Components │ │  TanStack Query    │    │   │
│  │  │ (TanStack  │ │  (shadcn/  │ │  + tRPC Client     │    │   │
│  │  │  Router)   │ │   React)   │ │                    │    │   │
│  │  └────────────┘ └────────────┘ └────────────────────┘    │   │
│  │  ┌────────────┐ ┌────────────┐ ┌────────────────────┐    │   │
│  │  │  tRPC v11  │ │ Vite 7 +   │ │  Tailwind CSS v4   │    │   │
│  │  │  Server    │ │ Nitro SSR  │ │                    │    │   │
│  │  └────────────┘ └────────────┘ └────────────────────┘    │   │
│  └──────────────────────────────────────────────────────────┘   │
│                              │ QStash (Upstash) /               │
...
│  │              Worker (Hono on Bun)                        │   │
│  │  ┌────────────┐ ┌────────────┐ ┌────────────────────┐    │   │
│  │  │  POST      │ │ Processors │ │  Vercel AI SDK     │    │   │
│  │  │ /process-  │ │ (doc,      │ │  (OpenAI + Google) │    │   │
│  │  │  file      │ │  image)    │ │                    │    │   │
│  │  └────────────┘ └────────────┘ └────────────────────┘    │   │
│  └──────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
...
The worker is a Hono server running on Bun that processes uploaded files. It exposes a single `POST /process-file` endpoint, invoked by the platform via Upstash QStash or bunqueue (for local development).
...
```
1. User Upload
   ├─ Browser → Platform (tRPC or /api/upload)
...
3. Background Processing (Worker)
   ├─ Queue invokes POST /process-file on Worker
   ├─ Worker downloads file from S3
   ├─ Content extracted (text from docs, description from images)
   ├─ Text chunked (800 tokens, 100 token overlap)
   ├─ Embeddings generated (1536-dimension vectors)
   ├─ Chunks + embeddings stored in fileContents table
   └─ File status updated to "completed"

4. Search Ready
   └─ File content is now searchable via vector similarity
```
...
1. Query Received
   ...
   └─ Platform receives tRPC request
...
4. Authorization
   ├─ Each request checks session + workspace membership
...
├─ Generate changesets from commits
├─ Version packages
