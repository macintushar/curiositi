...
| Word (.docx) | `application/vnd.openxmlformats-officedocument.wordprocessingml.document` | Full text extraction via mammoth, AI fallback |
| Word (.doc) | `application/msword` | AI-powered text extraction |
| PowerPoint (.pptx) | `application/vnd.openxmlformats-officedocument.presentationml.presentation` | Slide text extraction, AI fallback |
| PowerPoint (.ppt) | `application/vnd.ms-powerpoint` | AI-powered text extraction |
| Excel (.xlsx) | `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` | Tabular data extraction with sheet-aware chunking |
| Excel (.xls) | `application/vnd.ms-excel` | AI-powered text extraction |
| Plain Text | `text/plain` | Direct content indexing |
| Plaintext | `plaintext` | Direct content indexing |
| Markdown | `text/markdown` | Rendered text extracted |
| CSV | `text/csv` | Tabular data as text with header-aware chunking |
...
1. Upload
   ...
   └─ Processing job dispatched via QStash or bunqueue

2. Worker Processing (POST /process-file)
   ├─ Worker downloads file from S3
   ├─ Content extracted based on file type:
   │   ├─ Documents: text extraction (PDF, text, markdown, HTML, CSV, JSON, XML)
   │   ├─ Word (.docx): mammoth extraction with AI fallback
   ...
   ├─ Text chunked (300 tokens per chunk, 60 token overlap)
   ├─ Context prefix added (file name, type, page numbers, section titles)
...
2. If using QStash: verify `QSTASH_TOKEN`, `QSTASH_CURRENT_SIGNING_KEY`, and `QSTASH_NEXT_SIGNING_KEY` are set correctly in `.env`
3. If using bunqueue: verify `BUNQUEUE_HOST` and `BUNQUEUE_PORT` are set correctly and the bunqueue server is running
4. Verify `WORKER_URL` points to your worker instance (default: `http://localhost:3040`)
...
- [AI Search](/docs/features/ai-search) — Find your uploaded files
- [Spaces](/docs/features/spaces) — Organize files into spaces
