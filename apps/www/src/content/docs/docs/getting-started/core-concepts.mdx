...
### Agents
Agents are AI entities powered by LLMs (e.g., OpenAI, Anthropic, Google) configured with specific system prompts and tool access limits. They exist within a workspace and can use various tools to fulfill requests.

### Tools and MCP
Tools expand an agent's capabilities:
- **Built-in Tools**: Foundational actions (like semantic file search, web search).
