...
```
Files ←→ filesInSpace ←→ Spaces

report.pdf → filesInSpace → Marketing/Campaigns
report.pdf → filesInSpace → Finance/Reports
```
...
```
"Quarterly sales report" → [0.023, -0.156, 0.892, ...]  // 1536 dimensions
"Q4 revenue summary"     → [0.019, -0.142, 0.887, ...]  // Similar vector
```
...
```
┌─────────────┐     ┌──────────────┐     ┌─────────────┐
│   User      │────>│   Platform   │────>│   QStash    │
│  (Browser)  │     │  (TanStack   │     │  (Queue)    │
│             │     │   Start)     │     │             │
└─────────────┘     └──────────────┘     └──────┬──────┘
       ^                    │                    │
       │                    v                    v
